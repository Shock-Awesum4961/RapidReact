...
private final Timer intakeMotorTimer = new Timer(); // timer for intake motor to shutoff
  ...
  private static final int buttonIntakeMotor = 2;  // B
...
//    if(rumble1(shootdistanceOK))
  ...
  activateIntakeMotors(operatorJoystick.getRawButton(buttonIntakeMotor));
...
//  SmartDashboard.putNumber("tmr ", r_timer.get());
  ...
  if (intakeTrigger && !state3)  // 
    ...
    if (intakeDown == false)
      {
      intakeMotor.set(ControlMode.PercentOutput,-intakeSpeed); // start intake motor
      intakeSol.set(kForward); // lower intake
      intakeDown = true;
      }
    else
   ...
   if (climb1Trigger && !state4)  // 
    ...
    void activateIntakeMotors(boolean intakeMotorButton)
    {
      boolean manualIntakeMotor = false;
      if(intakeMotorButton){
        if(manualIntakeMotor){
          manualIntakeMotor = false;
          intakeMotor.stopMotor();
        }else{
          manualIntakeMotor = true;
          intakeMotor.set(ControlMode.PercentOutput,-intakeSpeed); // start intake motor
